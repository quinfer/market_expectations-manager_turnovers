...
spells |> distinct(name,club,Finish) |> nrow()
...
```{r}
#|label: fig-managertype
#|fig-cap: "Word cloud of manager types"

# Use the wordcloud package to create a word cloud of manager types using position column
library(wordcloud)
library(tm)
library(SnowballC)
...
paste0("(",paste(terms_to_remove,collapse="|"),")")  -> remove_terms
spells |>
  str_remove(position,terms_to_remove[1]) %>%
  {table(.$position)}
...
table(spells$staff_position[!is.na(spells$staff_position)]) |> sort(decreasing = TRUE)
```

```{r}

VCorpus <- Corpus(VectorSource(spells$position[!is.na(spells$position)]))
terms_to_remove <-c('Al-Ahli (UAE)','')
# Copy text to clipboard on Linux
text <- unique(spells$position[!is.na(spells$position)])
write(text, "| xclip -selection clipboard")

table(spells$position[!is.na(spells$position)])
VCorpus <- VCorpus |>
...
wordcloud(words = names(VCorpus), freq = VCorpus, min.freq = 1, scale = c(3, 0.5), colors = brewer.pal(8, "Dark2"))
```
...
spell |> mutate(season_start = ifelse(month(Start) < 8, year(Start), year(Start) + 1)) |> 
  count(season_start) |> 
...
# Define a function to make an HTTP request to the API and parse the response
function api_call() {
  local club_name="$1"
  local year_end_of_season="$2"
  local response=$(curl -s -H "Content-Type: application/json" \
    --data '{"model": "llama2", "prompt": "Which country is the club 
${club_name} from? What is its division level based on the year 
${year_end_of_season}?"}' http://localhost:11434/api/generate)
  echo "Processing club ${club_name} (${year_end_of_season})"
  echo "${response}" >> responses.csv
}

# Read the input CSV file with clubs and end of season dates using csvparse
input_file="input.csv"
output_file="output.csv"
IFS=$'\n'
mapfile -t clubs < <(csplit ${input_file} '{printf "%s\n" %${0}}}')
unset IFS
declare -a clubs_array=(${clubs[@]})
declare -a years_array=()
for i in $(seq 0 ${#clubs_array[*]}-1); do
  IFS=',' read -r club_name year < "${input_file}" || continue
  years_array+=("${year}")
done
unset IFS

# Loop over the clubs and call the API for each one with its respective end 
of season year
for ((i = 0; i < ${#clubs_array[@]}; i++)); do
  local club_name="${clubs_array[$i]}"
  local year="${years_array[$i]}"
  api_call "${club_name}" "${year}"
done

echo "Done processing all clubs. Total responses processed: $(wc -l < 
responses.csv)"
```
